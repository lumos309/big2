...
class ReplayBuffer():
    ...
    def sample(self, batch_size):
        ...
        '''
        # intialize the batch
        sampled_states = collections.deque()
        sampled_actions = collections.deque()
        sampled_rewards = collections.deque()
        sampled_next_states = collections.deque()
        sampled_dones = collections.deque()
        sample_idx = np.random.choice(len(self.buffer), batch_size, replace=False)

        # fetch random samples from the buffer
        for i in sample_idx:
            sampled_states.append(self.buffer[i].state)
            sampled_actions.append(self.buffer[i].action)
            sampled_rewards.append(self.buffer[i].reward)
            sampled_next_states.append(self.buffer[i].next_state)
            sampled_dones.append(self.buffer[i].done)

        states = torch.tensor(sampled_states, dtype=torch.float, device=device)
        actions = torch.tensor(sampled_actions, device=device)
        rewards = torch.tensor(sampled_rewards, dtype=torch.float, device=device)
        next_states = torch.tensor(sampled_next_states, dtype=torch.float, device=device)
        dones = torch.tensor(sampled_dones, dtype=torch.float, device=device)
...
class BaseAgent(Base):
    def act(self, state, epsilon=0.0):
        ...
        '''
        # Over a certain threshold, action will be random
        actions = self.forward(state)
        
        if random.uniform(0, 1) < epsilon:
          len = list(actions[0].size())[0]
          return int(np.random.choice(len, 1)[0]) # return random action
        else:
          max_index = torch.argmax(actions, dim=1).item()
          return int(max_index) # return action with max probability
...
def compute_loss(model, target, states, actions, rewards, next_states, dones):
    ...
    target = torch.max(target(next_states), dim=1).values
    target[dones.squeeze() == 1.0] = 0
    y_hat = (gamma * target) + rewards.squeeze() 
    y = torch.gather(model(states), 1, actions).squeeze()

    return loss_fn(y, y_hat) # difference between target and model

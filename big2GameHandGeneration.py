...
class big2Game:
    ...
    def reset(self):
        ...
        self.prevPlayedHands = {}
        self.prevPlayedHands[1] = np.zeros((53,), dtype=int)
        self.prevPlayedHands[2] = np.zeros((53,), dtype=int)
        self.prevPlayedHands[3] = np.zeros((53,), dtype=int)
        self.prevPlayedHands[4] = np.zeros((53,), dtype=int)
    ...
    def updateNeuralNetworkPass(self, cPlayer):
        for i in range(52):
            self.prevPlayedHands[cPlayer][i] = 0
        self.prevPlayedHands[cPlayer][52] = 1
    ...
    def updateNeuralNetworkInputs(self,prevHand, cPlayer):
        ...
        for card in prevHand:
            self.prevPlayedHands[cPlayer][card - 1] = 1
    ...
    def getHandPredictionState(self):
        currHands = [np.zeros((52,), dtype=int) for i in range(4)]
        for i in range(4):
            hand = self.currentHands[i + 1]
            for card in hand:
                currHands[i][card-1] = 1
        
        # input is player's hand + last four turns
        # target is opponents' current hands
        inp = currHands[self.playersGo - 1]
        inp = np.concatenate((inp, self.prevPlayedHands[1 + (self.playersGo) % 4], self.prevPlayedHands[1 + (self.playersGo + 1) % 4], self.prevPlayedHands[1 + (self.playersGo + 2) % 4]))

        tar = np.concatenate((currHands[self.playersGo % 4], currHands[(self.playersGo + 1) % 4], currHands[(self.playersGo + 2) % 4]))

        return inp, tar
    
    def getHandPredictionStateWithoutLabels(self):
        currHands = [np.zeros((52,), dtype=int) for i in range(4)]
        for i in range(4):
            hand = self.currentHands[i + 1]
            for card in hand:
...
def worker(remote, parent_remote):
    ...
    while True:
        ...
        elif cmd == "getCurrHands":
            currHands = game.getHandPredictionStateWithoutLabels()
            remote.send(currHands)
        elif cmd == 'close':
            remote.close()
            break
        else:
...
class vectorizedBig2Games(object):
    ...
    def currHands_async(self):
        for remote in self.remotes:
            remote.send(('getCurrHands', None))
        self.waiting = True
        
    def currHands_wait(self):
        results = [remote.recv() for remote in self.remotes]
        self.waiting = False
        # currHands = zip(*results)
        # return np.stack(currHands)
        return results
    ...
    def getCurrHands(self):
        self.currHands_async()
        return self.currHands_wait()

...
from PPONetworkHandProb import PPONetwork, PPOModel
...
class big2PPOSimulation(object):
    ...
    def __init__(self, sess, sessConfig=None, startingUpdate = 0, osStartingUpdate = 0, inpDim = 412 + 156, nGames = 8, nSteps = 20, nMiniBatches = 4, nOptEpochs = 5, lam = 0.95, gamma = 0.995, ent_coef = 0.01, vf_coef = 0.5, max_grad_norm = 0.5, minLearningRate = 0.000001, learningRate=0.00025, clipRange=0.2, saveEvery = 500, oppSamplingRate = 0.0, eta = 0.1):
        ...
        if startingUpdate > 0:
            self.trainingNetwork.loadParams(joblib.load('handPredictionParameters' + str(startingUpdate)))
        ...
        #     self.handPredictionModelInputs = tf.compat.v1.placeholder(tf.float32, (self.nGames, 211))
        #     self.currHandPrediction = self.handPredictionModel.predictBatch(handPredictionModelInputs)
    ...
    def train(self, nTotalSteps):
        ...
        for update in range(self.startingUpdate + 1, nUpdates):
            ...
            states, availAcs, returns, actions, values, neglogpacs = run()
            ...
            if update % self.saveEvery == 0:
                name = "handPredictionParameters" + str(update)
